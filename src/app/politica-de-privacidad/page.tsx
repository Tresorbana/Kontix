'use client';

import Navigation from "@/components/sections/navigation";
import Footer from "@/components/sections/footer";
import { useLanguage } from '@/contexts/LanguageContext';

export default function PoliticaDePrivacidadPage() {
  const { t } = useLanguage();
  return (
    <main className="bg-black text-white min-h-screen">
      <Navigation />
      <section className="pt-28 sm:pt-32 pb-20">
        <div className="container mx-auto px-4 max-w-3xl space-y-6">
          <header className="space-y-2">
            <h1 className="text-3xl font-bold">Caribbean Ventures</h1>
            <h2 className="text-2xl font-semibold">{t('privacy.title')}</h2>
          </header>

          <p>
            {t('privacy.intro')}
          </p>

          <section className="space-y-3">
            <h3 className="text-xl font-semibold">{t('privacy.sections.collection.title')}</h3>
            <p>
              {t('privacy.sections.collection.description')}
            </p>
          </section>

          <section className="space-y-3">
            <h3 className="text-xl font-semibold">{t('privacy.sections.security.title')}</h3>
            <p>
              {t('privacy.sections.security.description')}
            </p>
          </section>

          <section className="space-y-3">
            <h3 className="text-xl font-semibold">{t('privacy.sections.acceptance.title')}</h3>
            <p>
              {t('privacy.sections.acceptance.description')}
            </p>
          </section>

          <section className="space-y-3">
            <h3 className="text-xl font-semibold">{t('footer.contactTitle')}</h3>
            <p>
              {t('privacy.sections.security.description')}
            </p>
            <p className="text-sm text-gray-300">
              {t('footer.address')}
              <br />
              {t('contact.emailLabel')}: contact@caribbean.ventures
            </p>
          </section>
        </div>
      </section>
      <Footer />
    </main>
  );
}
